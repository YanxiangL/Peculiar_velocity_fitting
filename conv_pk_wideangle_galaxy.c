...
int main(int argc, char **argv) {
	...
	double diag_mm_1_12 = conv_integral(kmin, kmax, 0.0, 0, 0, 12, 1, 0)*(H_dd(0, 4, 3, theta_diag, phi_diag) + H_dd(0, 3, 4, theta_diag, phi_diag));
	double diag_mm_1_10 = conv_integral(kmin, kmax, 0.0, 0, 0, 10, 1, 0)*(H_dd(0, 4, 2, theta_diag, phi_diag) + 2.0*H_dd(0, 3, 3, theta_diag, phi_diag) + H_dd(0, 2, 4, theta_diag, phi_diag));
	double diag_mm_1_8  = conv_integral(kmin, kmax, 0.0, 0, 0, 8, 1, 0)*(H_dd(0, 4, 1, theta_diag, phi_diag)/6.0 + 5.0*H_dd(0, 3, 2, theta_diag, phi_diag)/12.0 + 5.0*H_dd(0, 2, 3, theta_diag, phi_diag)/12.0 + H_dd(0, 1, 4, theta_diag, phi_diag)/6.0);
	double diag_mm_1_6  = conv_integral(kmin, kmax, 0.0, 0, 0, 6, 1, 0)*(H_dd(0, 4, 0, theta_diag, phi_diag)/6.0 + 2.0*H_dd(0, 3, 1, theta_diag, phi_diag)/3.0 + H_dd(0, 2, 2, theta_diag, phi_diag) + 2.0*H_dd(0, 1, 3, theta_diag, phi_diag)/3.0 + H_dd(0, 0, 4, theta_diag, phi_diag)/6.0);
	double diag_mm_1_4  = conv_integral(kmin, kmax, 0.0, 0, 0, 4, 1, 0)*(H_dd(0, 3, 0, theta_diag, phi_diag)/2.0 + 3.0*H_dd(0, 2, 1, theta_diag, phi_diag)/2.0 + 3.0*H_dd(0, 1, 2, theta_diag, phi_diag)/2.0 + H_dd(0, 0, 3, theta_diag, phi_diag)/2.0);
	double diag_mm_1_2  = conv_integral(kmin, kmax, 0.0, 0, 0, 2, 1, 0)*(H_dd(0, 2, 0, theta_diag, phi_diag) + 2.0*H_dd(0, 1, 1, theta_diag, phi_diag) + H_dd(0, 0, 2, theta_diag, phi_diag));
	double diag_mm_1_0  = conv_integral(kmin, kmax, 0.0, 0, 0, 0, 1, 0)*(H_dd(0, 1, 0, theta_diag, phi_diag) + H_dd(0, 0, 1, theta_diag, phi_diag));
	double diag_mm_2_12 = conv_integral(kmin, kmax, 0.0, 0, 0, 12, 1, 0)*H_dd(0, 4, 4, theta_diag, phi_diag);
	double diag_mm_2_10 = conv_integral(kmin, kmax, 0.0, 0, 0, 10, 1, 0)*(H_dd(0, 4, 3, theta_diag, phi_diag) + H_dd(0, 3, 4, theta_diag, phi_diag));
	double diag_mm_2_8  = conv_integral(kmin, kmax, 0.0, 0, 0, 8, 1, 0)*(H_dd(0, 4, 2, theta_diag, phi_diag)/6.0 + H_dd(0, 3, 3, theta_diag, phi_diag)/4.0 + H_dd(0, 2, 4, theta_diag, phi_diag)/6.0);
	double diag_mm_2_6  = conv_integral(kmin, kmax, 0.0, 0, 0, 6, 1, 0)*(H_dd(0, 4, 1, theta_diag, phi_diag)/6.0 + H_dd(0, 3, 2, theta_diag, phi_diag)/2.0 + H_dd(0, 2, 3, theta_diag, phi_diag)/2.0 + H_dd(0, 1, 4, theta_diag, phi_diag)/6.0);
	double diag_mm_2_4  = conv_integral(kmin, kmax, 0.0, 0, 0, 4, 1, 0)*(H_dd(0, 3, 1, theta_diag, phi_diag)/2.0 + H_dd(0, 2, 2, theta_diag, phi_diag) + H_dd(0, 1, 3, theta_diag, phi_diag)/2.0);
	double diag_mm_2_2  = conv_integral(kmin, kmax, 0.0, 0, 0, 2, 1, 0)*(H_dd(0, 2, 1, theta_diag, phi_diag) + H_dd(0, 1, 2, theta_diag, phi_diag));
	double diag_mm_2_0  = conv_integral(kmin, kmax, 0.0, 0, 0, 0, 1, 0)*H_dd(0, 1, 1, theta_diag, phi_diag);
