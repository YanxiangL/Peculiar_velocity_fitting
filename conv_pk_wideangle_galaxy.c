...
// Code to calculate the covariance matrix for fitting the peculiar velocities of the SDSS survey.
...
double conv_integral(double kmin, double kmax, double dist, int abtype, int ell, int pq, int gridcorr, int damping) {//Calculate the integration over the power spectra. 
...
int main(int argc, char **argv) {
	...
	double job_num = atof(argv[2]); //Enter a negative number if you just want to check whether the configuration file is being read correctly without running the code. 
    ...
    //sprintf(gridcorrfile, "./grid_correction/gridcorr_%d.dat", gridsize);
	//sprintf(gridcorrfile, gridcorrfile, gridsize);
	...
	double phi_diag = 0.0; 
	//At the diagonal of the matrix, line of sight to two galaxies overlap, so theta = 0; The diagonal does not depend on the angle phi. For the diagonal component,
	//only the monopole survives. 
	double diag_mm_0_12 = conv_integral(kmin, kmax, 0.0, 0, 0, 12, 1, 0)*H_dd(0, 3, 3, theta_diag, phi_diag);
	double diag_mm_0_10 = conv_integral(kmin, kmax, 0.0, 0, 0, 10, 1, 0)*(H_dd(0, 3, 2, theta_diag, phi_diag) + H_dd(0, 2, 3, theta_diag, phi_diag));
	double diag_mm_0_8  = conv_integral(kmin, kmax, 0.0, 0, 0, 8, 1, 0)*(H_dd(0, 3, 1, theta_diag, phi_diag)/6.0 + H_dd(0, 2, 2, theta_diag, phi_diag)/4.0 + H_dd(0, 1, 3, theta_diag, phi_diag)/6.0);
	double diag_mm_0_6  = conv_integral(kmin, kmax, 0.0, 0, 0, 6, 1, 0)*(H_dd(0, 3, 0, theta_diag, phi_diag)/6.0 + H_dd(0, 2, 1, theta_diag, phi_diag)/2.0 + H_dd(0, 1, 2, theta_diag, phi_diag)/2.0 + H_dd(0, 0, 3, theta_diag, phi_diag)/6.0);
	double diag_mm_0_4  = conv_integral(kmin, kmax, 0.0, 0, 0, 4, 1, 0)*(H_dd(0, 2, 0, theta_diag, phi_diag)/2.0 + H_dd(0, 1, 1, theta_diag, phi_diag) + H_dd(0, 0, 2, theta_diag, phi_diag)/2.0);
	double diag_mm_0_2  = conv_integral(kmin, kmax, 0.0, 0, 0, 2, 1, 0)*(H_dd(0, 1, 0, theta_diag, phi_diag) + H_dd(0, 0, 1, theta_diag, phi_diag));
	double diag_mm_0_0  = conv_integral(kmin, kmax, 0.0, 0, 0, 0, 1, 0)*H_dd(0, 0, 0, theta_diag, phi_diag);
	double diag_mm_1_12 = conv_integral(kmin, kmax, 0.0, 1, 0, 12, 1, 0)*(H_dd(0, 4, 3, theta_diag, phi_diag) + H_dd(0, 3, 4, theta_diag, phi_diag));
	double diag_mm_1_10 = conv_integral(kmin, kmax, 0.0, 1, 0, 10, 1, 0)*(H_dd(0, 4, 2, theta_diag, phi_diag) + 2.0*H_dd(0, 3, 3, theta_diag, phi_diag) + H_dd(0, 2, 4, theta_diag, phi_diag));
	double diag_mm_1_8  = conv_integral(kmin, kmax, 0.0, 1, 0, 8, 1, 0)*(H_dd(0, 4, 1, theta_diag, phi_diag)/6.0 + 5.0*H_dd(0, 3, 2, theta_diag, phi_diag)/12.0 + 5.0*H_dd(0, 2, 3, theta_diag, phi_diag)/12.0 + H_dd(0, 1, 4, theta_diag, phi_diag)/6.0);
	double diag_mm_1_6  = conv_integral(kmin, kmax, 0.0, 1, 0, 6, 1, 0)*(H_dd(0, 4, 0, theta_diag, phi_diag)/6.0 + 2.0*H_dd(0, 3, 1, theta_diag, phi_diag)/3.0 + H_dd(0, 2, 2, theta_diag, phi_diag) + 2.0*H_dd(0, 1, 3, theta_diag, phi_diag)/3.0 + H_dd(0, 0, 4, theta_diag, phi_diag)/6.0);
	double diag_mm_1_4  = conv_integral(kmin, kmax, 0.0, 1, 0, 4, 1, 0)*(H_dd(0, 3, 0, theta_diag, phi_diag)/2.0 + 3.0*H_dd(0, 2, 1, theta_diag, phi_diag)/2.0 + 3.0*H_dd(0, 1, 2, theta_diag, phi_diag)/2.0 + H_dd(0, 0, 3, theta_diag, phi_diag)/2.0);
	double diag_mm_1_2  = conv_integral(kmin, kmax, 0.0, 1, 0, 2, 1, 0)*(H_dd(0, 2, 0, theta_diag, phi_diag) + 2.0*H_dd(0, 1, 1, theta_diag, phi_diag) + H_dd(0, 0, 2, theta_diag, phi_diag));
	double diag_mm_1_0  = conv_integral(kmin, kmax, 0.0, 1, 0, 0, 1, 0)*(H_dd(0, 1, 0, theta_diag, phi_diag) + H_dd(0, 0, 1, theta_diag, phi_diag));
	double diag_mm_2_12 = conv_integral(kmin, kmax, 0.0, 2, 0, 12, 1, 0)*H_dd(0, 4, 4, theta_diag, phi_diag);
	double diag_mm_2_10 = conv_integral(kmin, kmax, 0.0, 2, 0, 10, 1, 0)*(H_dd(0, 4, 3, theta_diag, phi_diag) + H_dd(0, 3, 4, theta_diag, phi_diag));
	double diag_mm_2_8  = conv_integral(kmin, kmax, 0.0, 2, 0, 8, 1, 0)*(H_dd(0, 4, 2, theta_diag, phi_diag)/6.0 + H_dd(0, 3, 3, theta_diag, phi_diag)/4.0 + H_dd(0, 2, 4, theta_diag, phi_diag)/6.0);
	double diag_mm_2_6  = conv_integral(kmin, kmax, 0.0, 2, 0, 6, 1, 0)*(H_dd(0, 4, 1, theta_diag, phi_diag)/6.0 + H_dd(0, 3, 2, theta_diag, phi_diag)/2.0 + H_dd(0, 2, 3, theta_diag, phi_diag)/2.0 + H_dd(0, 1, 4, theta_diag, phi_diag)/6.0);
	double diag_mm_2_4  = conv_integral(kmin, kmax, 0.0, 2, 0, 4, 1, 0)*(H_dd(0, 3, 1, theta_diag, phi_diag)/2.0 + H_dd(0, 2, 2, theta_diag, phi_diag) + H_dd(0, 1, 3, theta_diag, phi_diag)/2.0);
	double diag_mm_2_2  = conv_integral(kmin, kmax, 0.0, 2, 0, 2, 1, 0)*(H_dd(0, 2, 1, theta_diag, phi_diag) + H_dd(0, 1, 2, theta_diag, phi_diag));
	double diag_mm_2_0  = conv_integral(kmin, kmax, 0.0, 2, 0, 0, 1, 0)*H_dd(0, 1, 1, theta_diag, phi_diag);

	//*****************************************************************************************//
    int rbins = 10000;
	double * svals = (double *)malloc(rbins*sizeof(double));
	double * xivals = (double *)malloc(rbins*sizeof(double));
   	printf("Precomputing dd correlation functions\n");

	xi_dd_acc = (gsl_interp_accel ****)malloc(3*sizeof(gsl_interp_accel ***));
	for (i=0; i<3; i++) {
		xi_dd_acc[i] = (gsl_interp_accel ***)malloc(7*sizeof(gsl_interp_accel **));
		for (j=0; j<7; j++) xi_dd_acc[i][j] = (gsl_interp_accel **)malloc((j+3)*sizeof(gsl_interp_accel *));
	}
	xi_dd_spline = (gsl_spline ****)malloc(3*sizeof(gsl_spline ***));
	for (i=0; i<3; i++) {
		xi_dd_spline[i] = (gsl_spline ***)malloc(7*sizeof(gsl_spline **));
		for (j=0; j<7; j++) xi_dd_spline[i][j] = (gsl_spline **)malloc((j+3)*sizeof(gsl_spline *));
	}
    
	//Calculate and interpolate the integral of equation (16) in the paper. 
	...
	for (veltype=0; veltype<3; veltype++) { //veltype determines which power spectrum to use.
		for (i=0; i<7; i++) {// 2*i determines the power of k in the integral.
			for (ell=0; ell<i+3; ell++) { //2*ell is the order of the spherical bessel function.
				for (j=0; j<rbins; j++) xivals[j] = conv_integral(kmin, kmax, svals[j], veltype, 2*ell, 2*i, 1, 0);
				printf("%d, %d, %d\n", veltype, 2*i, 2*ell);
				xi_dd_acc[veltype][i][ell] = gsl_interp_accel_alloc();
   	 			xi_dd_spline[veltype][i][ell] = gsl_spline_alloc(gsl_interp_cspline, rbins);
    			gsl_spline_init(xi_dd_spline[veltype][i][ell], svals, xivals, rbins);
			}
		}
	}
	free(svals);
	free(xivals);
   	...
   	printf("Starting dd covariance\n");
	double ** conv_pk_gal_0_0 = (double **)malloc(nelements*sizeof(double*));
	double ** conv_pk_gal_0_2 = (double **)malloc(nelements*sizeof(double*));
	double ** conv_pk_gal_0_4 = (double **)malloc(nelements*sizeof(double*));
	double ** conv_pk_gal_0_6 = (double **)malloc(nelements*sizeof(double*));
	double ** conv_pk_gal_0_8 = (double **)malloc(nelements*sizeof(double*));
	double ** conv_pk_gal_0_10 = (double **)malloc(nelements*sizeof(double*));
	double ** conv_pk_gal_0_12 = (double **)malloc(nelements*sizeof(double*));
    double ** conv_pk_gal_1_0 = (double **)malloc(nelements*sizeof(double*));
    double ** conv_pk_gal_1_2 = (double **)malloc(nelements*sizeof(double*));
    double ** conv_pk_gal_1_4 = (double **)malloc(nelements*sizeof(double*));
    double ** conv_pk_gal_1_6 = (double **)malloc(nelements*sizeof(double*));
    double ** conv_pk_gal_1_8 = (double **)malloc(nelements*sizeof(double*));
    double ** conv_pk_gal_1_10 = (double **)malloc(nelements*sizeof(double*));
    double ** conv_pk_gal_1_12 = (double **)malloc(nelements*sizeof(double*));
    double ** conv_pk_gal_2_0 = (double **)malloc(nelements*sizeof(double*));
    double ** conv_pk_gal_2_2 = (double **)malloc(nelements*sizeof(double*));
    double ** conv_pk_gal_2_4 = (double **)malloc(nelements*sizeof(double*));
    double ** conv_pk_gal_2_6 = (double **)malloc(nelements*sizeof(double*));
    double ** conv_pk_gal_2_8 = (double **)malloc(nelements*sizeof(double*));
    double ** conv_pk_gal_2_10 = (double **)malloc(nelements*sizeof(double*));
    double ** conv_pk_gal_2_12 = (double **)malloc(nelements*sizeof(double*));
    for (i=0; i<nelements; i++) {
		conv_pk_gal_0_0[i] = (double *)malloc(nelements*sizeof(double));
		conv_pk_gal_0_2[i] = (double *)malloc(nelements*sizeof(double));
		conv_pk_gal_0_4[i] = (double *)malloc(nelements*sizeof(double));
		conv_pk_gal_0_6[i] = (double *)malloc(nelements*sizeof(double));
		conv_pk_gal_0_8[i] = (double *)malloc(nelements*sizeof(double));
		conv_pk_gal_0_10[i] = (double *)malloc(nelements*sizeof(double));
		conv_pk_gal_0_12[i] = (double *)malloc(nelements*sizeof(double));
        conv_pk_gal_1_0[i] = (double *)malloc(nelements*sizeof(double));
        conv_pk_gal_1_2[i] = (double *)malloc(nelements*sizeof(double));
        conv_pk_gal_1_4[i] = (double *)malloc(nelements*sizeof(double));
        conv_pk_gal_1_6[i] = (double *)malloc(nelements*sizeof(double));
        conv_pk_gal_1_8[i] = (double *)malloc(nelements*sizeof(double));
        conv_pk_gal_1_10[i] = (double *)malloc(nelements*sizeof(double));
        conv_pk_gal_1_12[i] = (double *)malloc(nelements*sizeof(double));
        conv_pk_gal_2_0[i] = (double *)malloc(nelements*sizeof(double));
        conv_pk_gal_2_2[i] = (double *)malloc(nelements*sizeof(double));
        conv_pk_gal_2_4[i] = (double *)malloc(nelements*sizeof(double));
        conv_pk_gal_2_6[i] = (double *)malloc(nelements*sizeof(double));
        conv_pk_gal_2_8[i] = (double *)malloc(nelements*sizeof(double));
        conv_pk_gal_2_10[i] = (double *)malloc(nelements*sizeof(double));
        conv_pk_gal_2_12[i] = (double *)malloc(nelements*sizeof(double));
    }
	...
	for (i=0; i<niter; i++) {
        ...
        int indj = indexj[i];
        if (indi == indj) {
            conv_pk_gal_0_0[indi][indi]  =  diag_mm_0_0/(2.0*M_PI*M_PI);
            conv_pk_gal_0_2[indi][indi]  = -diag_mm_0_2/2.0/(2.0*M_PI*M_PI);
            conv_pk_gal_0_4[indi][indi]  =  diag_mm_0_4/4.0/(2.0*M_PI*M_PI);
            conv_pk_gal_0_6[indi][indi]  = -diag_mm_0_6/8.0/(2.0*M_PI*M_PI);
            conv_pk_gal_0_8[indi][indi]  =  diag_mm_0_8/16.0/(2.0*M_PI*M_PI);
            conv_pk_gal_0_10[indi][indi] = -diag_mm_0_10/384.0/(2.0*M_PI*M_PI);
            conv_pk_gal_0_12[indi][indi] =  diag_mm_0_12/2304.0/(2.0*M_PI*M_PI);
            conv_pk_gal_1_0[indi][indi]  =  diag_mm_1_0/(2.0*M_PI*M_PI);
            conv_pk_gal_1_2[indi][indi]  = -diag_mm_1_2/2.0/(2.0*M_PI*M_PI);
            conv_pk_gal_1_4[indi][indi]  =  diag_mm_1_4/4.0/(2.0*M_PI*M_PI);
            conv_pk_gal_1_6[indi][indi]  = -diag_mm_1_6/8.0/(2.0*M_PI*M_PI);
            conv_pk_gal_1_8[indi][indi]  =  diag_mm_1_8/16.0/(2.0*M_PI*M_PI);
            conv_pk_gal_1_10[indi][indi] = -diag_mm_1_10/384.0/(2.0*M_PI*M_PI);
            conv_pk_gal_1_12[indi][indi] =  diag_mm_1_12/2304.0/(2.0*M_PI*M_PI);
            conv_pk_gal_2_0[indi][indi]  =  diag_mm_2_0/(2.0*M_PI*M_PI);
            conv_pk_gal_2_2[indi][indi]  = -diag_mm_2_2/2.0/(2.0*M_PI*M_PI);
            conv_pk_gal_2_4[indi][indi]  =  diag_mm_2_4/4.0/(2.0*M_PI*M_PI);
            conv_pk_gal_2_6[indi][indi]  = -diag_mm_2_6/8.0/(2.0*M_PI*M_PI);
            conv_pk_gal_2_8[indi][indi]  =  diag_mm_2_8/16.0/(2.0*M_PI*M_PI);
            conv_pk_gal_2_10[indi][indi] = -diag_mm_2_10/384.0/(2.0*M_PI*M_PI);
            conv_pk_gal_2_12[indi][indi] =  diag_mm_2_12/2304.0/(2.0*M_PI*M_PI);
            printf("%d, %lf, %lf, %lf, %lf, %lf, %lf\n", indi, datagrid_x[indi], datagrid_y[indi], datagrid_z[indi], conv_pk_gal_0_0[indi][indi], 1.0e6*conv_pk_gal_1_6[indi][indi], 1.0e6*conv_pk_gal_2_12[indi][indi]);
        } else {
			...
			double cov_1_0 = 0, cov_1_2 = 0, cov_1_4 = 0, cov_1_6 = 0, cov_1_8 = 0, cov_1_10 = 0, cov_1_12 = 0;
			double cov_2_0 = 0, cov_2_2 = 0, cov_2_4 = 0, cov_2_6 = 0, cov_2_8 = 0, cov_2_10 = 0, cov_2_12 = 0;
			for (ell=16; ell>=0; ell-=2) {
				...
				cov_1_12 += creal(cpow(I, ell))*gsl_spline_eval(xi_dd_spline[1][6][ell/2], s, xi_dd_acc[1][6][ell/2])*(H_dd(ell, 4, 3, theta, phi) + H_dd(ell, 3, 4, theta, phi));
				cov_2_12 += creal(cpow(I, ell))*gsl_spline_eval(xi_dd_spline[2][6][ell/2], s, xi_dd_acc[2][6][ell/2])*H_dd(ell, 4, 4, theta, phi);
				if (ell < 15) {
					...
					cov_1_10 += creal(cpow(I, ell))*gsl_spline_eval(xi_dd_spline[1][5][ell/2], s, xi_dd_acc[1][5][ell/2])*(H_dd(ell, 4, 2, theta, phi) + 2.0*H_dd(ell, 3, 3, theta, phi) + H_dd(ell, 2, 4, theta, phi));
					cov_2_10 += creal(cpow(I, ell))*gsl_spline_eval(xi_dd_spline[2][5][ell/2], s, xi_dd_acc[2][5][ell/2])*(H_dd(ell, 4, 3, theta, phi) + H_dd(ell, 3, 4, theta, phi));
				}
				if (ell < 13) {
					...
					cov_1_8 += creal(cpow(I, ell))*gsl_spline_eval(xi_dd_spline[1][4][ell/2], s, xi_dd_acc[1][4][ell/2])*(H_dd(ell, 4, 1, theta, phi)/6.0 + 5.0*H_dd(ell, 3, 2, theta, phi)/12.0 + 5.0*H_dd(ell, 2, 3, theta, phi)/12.0 + H_dd(ell, 1, 4, theta, phi)/6.0);
					cov_2_8 += creal(cpow(I, ell))*gsl_spline_eval(xi_dd_spline[2][4][ell/2], s, xi_dd_acc[2][4][ell/2])*(H_dd(ell, 4, 2, theta, phi)/6.0 + H_dd(ell, 3, 3, theta, phi)/4.0 + H_dd(ell, 2, 4, theta, phi)/6.0);
				}
				if (ell < 11) {
					...
					cov_1_6 += creal(cpow(I, ell))*gsl_spline_eval(xi_dd_spline[1][3][ell/2], s, xi_dd_acc[1][3][ell/2])*(H_dd(ell, 4, 0, theta, phi)/6.0 + 2.0*H_dd(ell, 3, 1, theta, phi)/3.0 + H_dd(ell, 2, 2, theta, phi) + 2.0*H_dd(ell, 1, 3, theta, phi)/3.0 + H_dd(ell, 0, 4, theta, phi)/6.0);
					cov_2_6 += creal(cpow(I, ell))*gsl_spline_eval(xi_dd_spline[2][3][ell/2], s, xi_dd_acc[2][3][ell/2])*(H_dd(ell, 4, 1, theta, phi)/6.0 + H_dd(ell, 3, 2, theta, phi)/2.0 + H_dd(ell, 2, 3, theta, phi)/2.0 + H_dd(ell, 1, 4, theta, phi)/6.0);
				}
				if (ell < 9) {
					...
					cov_1_4 += creal(cpow(I, ell))*gsl_spline_eval(xi_dd_spline[1][2][ell/2], s, xi_dd_acc[1][2][ell/2])*(H_dd(ell, 3, 0, theta, phi)/2.0 + 3.0*H_dd(ell, 2, 1, theta, phi)/2.0 + 3.0*H_dd(ell, 1, 2, theta, phi)/2.0 + H_dd(ell, 0, 3, theta, phi)/2.0);
					cov_2_4 += creal(cpow(I, ell))*gsl_spline_eval(xi_dd_spline[2][2][ell/2], s, xi_dd_acc[2][2][ell/2])*(H_dd(ell, 3, 1, theta, phi)/2.0 + H_dd(ell, 2, 2, theta, phi) + H_dd(ell, 1, 3, theta, phi)/2.0);
				}
				if (ell < 7) {
					...
					cov_1_2 += creal(cpow(I, ell))*gsl_spline_eval(xi_dd_spline[1][1][ell/2], s, xi_dd_acc[1][1][ell/2])*(H_dd(ell, 2, 0, theta, phi) + 2.0*H_dd(ell, 1, 1, theta, phi) + H_dd(ell, 0, 2, theta, phi));
					cov_2_2 += creal(cpow(I, ell))*gsl_spline_eval(xi_dd_spline[2][1][ell/2], s, xi_dd_acc[2][1][ell/2])*(H_dd(ell, 2, 1, theta, phi) + H_dd(ell, 1, 2, theta, phi));
				}
				if (ell < 5) {
					cov_0_0 += creal(cpow(I, ell))*gsl_spline_eval(xi_dd_spline[0][0][ell/2], s, xi_dd_acc[0][0][ell/2])*H_dd(ell, 0, 0, theta, phi);
					cov_1_0 += creal(cpow(I, ell))*gsl_spline_eval(xi_dd_spline[1][0][ell/2], s, xi_dd_acc[1][0][ell/2])*(H_dd(ell, 1, 0, theta, phi) + H_dd(ell, 0, 1, theta, phi));
					cov_2_0 += creal(cpow(I, ell))*gsl_spline_eval(xi_dd_spline[2][0][ell/2], s, xi_dd_acc[2][0][ell/2])*H_dd(ell, 1, 1, theta, phi);
				}
			}
            ...
            conv_pk_gal_1_12[indi][indj] =  cov_1_12/(2.0*M_PI*M_PI)/2304.0;
            conv_pk_gal_1_10[indi][indj] = -cov_1_10/(2.0*M_PI*M_PI)/384.0;
            conv_pk_gal_1_8[indi][indj]  =  cov_1_8/(2.0*M_PI*M_PI)/16.0;
            conv_pk_gal_1_6[indi][indj]  = -cov_1_6/(2.0*M_PI*M_PI)/8.0;
            conv_pk_gal_1_4[indi][indj]  =  cov_1_4/(2.0*M_PI*M_PI)/4.0;
            conv_pk_gal_1_2[indi][indj]  = -cov_1_2/(2.0*M_PI*M_PI)/2.0;
            conv_pk_gal_1_0[indi][indj]  =  cov_1_0/(2.0*M_PI*M_PI);
            conv_pk_gal_2_12[indi][indj] =  cov_2_12/(2.0*M_PI*M_PI)/2304.0;
            conv_pk_gal_2_10[indi][indj] = -cov_2_10/(2.0*M_PI*M_PI)/384.0;
            conv_pk_gal_2_8[indi][indj]  =  cov_2_8/(2.0*M_PI*M_PI)/16.0;
            conv_pk_gal_2_6[indi][indj]  = -cov_2_6/(2.0*M_PI*M_PI)/8.0;
            conv_pk_gal_2_4[indi][indj]  =  cov_2_4/(2.0*M_PI*M_PI)/4.0;
            conv_pk_gal_2_2[indi][indj]  = -cov_2_2/(2.0*M_PI*M_PI)/2.0;
            conv_pk_gal_2_0[indi][indj]  =  cov_2_0/(2.0*M_PI*M_PI);
            ...
            conv_pk_gal_1_12[indj][indi] =  cov_1_12/(2.0*M_PI*M_PI)/2304.0;
            conv_pk_gal_1_10[indj][indi] = -cov_1_10/(2.0*M_PI*M_PI)/384.0;
            conv_pk_gal_1_8[indj][indi]  =  cov_1_8/(2.0*M_PI*M_PI)/16.0;
            conv_pk_gal_1_6[indj][indi]  = -cov_1_6/(2.0*M_PI*M_PI)/8.0;
            conv_pk_gal_1_4[indj][indi]  =  cov_1_4/(2.0*M_PI*M_PI)/4.0;
            conv_pk_gal_1_2[indj][indi]  = -cov_1_2/(2.0*M_PI*M_PI)/2.0;
            conv_pk_gal_1_0[indj][indi]  =  cov_1_0/(2.0*M_PI*M_PI);
            conv_pk_gal_2_12[indj][indi] =  cov_2_12/(2.0*M_PI*M_PI)/2304.0;
            conv_pk_gal_2_10[indj][indi] = -cov_2_10/(2.0*M_PI*M_PI)/384.0;
            conv_pk_gal_2_8[indj][indi]  =  cov_2_8/(2.0*M_PI*M_PI)/16.0;
            conv_pk_gal_2_6[indj][indi]  = -cov_2_6/(2.0*M_PI*M_PI)/8.0;
            conv_pk_gal_2_4[indj][indi]  =  cov_2_4/(2.0*M_PI*M_PI)/4.0;
            conv_pk_gal_2_2[indj][indi]  = -cov_2_2/(2.0*M_PI*M_PI)/2.0;
            conv_pk_gal_2_0[indj][indi]  =  cov_2_0/(2.0*M_PI*M_PI);
    ...
    write_cov(covfile, nelements, conv_pk_gal_1_0, 8);
	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_1_2.dat", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize);
    write_cov(covfile, nelements, conv_pk_gal_1_2, 10);
	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_1_4.dat", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize);
    write_cov(covfile, nelements, conv_pk_gal_1_4, 12);
	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_1_6.dat", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize);
    write_cov(covfile, nelements, conv_pk_gal_1_6, 14);
 	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_1_8.dat", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize);
    write_cov(covfile, nelements, conv_pk_gal_1_8, 16);
 	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_1_10.dat", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize);
    write_cov(covfile, nelements, conv_pk_gal_1_10, 18);
 	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_1_12.dat", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize);
    write_cov(covfile, nelements, conv_pk_gal_1_12, 20);
	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_2_0.dat", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize);
    write_cov(covfile, nelements, conv_pk_gal_2_0, 8);
	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_2_2.dat", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize);
    write_cov(covfile, nelements, conv_pk_gal_2_2, 10);
	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_2_4.dat", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize);
    write_cov(covfile, nelements, conv_pk_gal_2_4, 12);
	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_2_6.dat", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize);
    write_cov(covfile, nelements, conv_pk_gal_2_6, 14);
 	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_2_8.dat", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize);
    write_cov(covfile, nelements, conv_pk_gal_2_8, 16);
 	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_2_10.dat", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize);
    write_cov(covfile, nelements, conv_pk_gal_2_10, 18);
 	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_2_12.dat", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize);
    write_cov(covfile, nelements, conv_pk_gal_2_12, 20);*/
	
	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_0_0.bin", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize);
    write_cov(covfile, nelements, conv_pk_gal_0_0, 8);
	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_0_2.bin", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize);
    write_cov(covfile, nelements, conv_pk_gal_0_2, 10);
	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_0_4.bin", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize);
    write_cov(covfile, nelements, conv_pk_gal_0_4, 12);
	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_0_6.bin", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize);
    write_cov(covfile, nelements, conv_pk_gal_0_6, 14);
 	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_0_8.bin", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize);
    write_cov(covfile, nelements, conv_pk_gal_0_8, 16);
 	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_0_10.bin", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize);
    write_cov(covfile, nelements, conv_pk_gal_0_10, 18);
 	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_0_12.bin", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize);
    write_cov(covfile, nelements, conv_pk_gal_0_12, 20);
	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_1_0.bin", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize);
    write_cov(covfile, nelements, conv_pk_gal_1_0, 8);
	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_1_2.bin", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize);
    write_cov(covfile, nelements, conv_pk_gal_1_2, 10);
	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_1_4.bin", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize);
    write_cov(covfile, nelements, conv_pk_gal_1_4, 12);
	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_1_6.bin", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize);
    write_cov(covfile, nelements, conv_pk_gal_1_6, 14);
 	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_1_8.bin", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize);
    write_cov(covfile, nelements, conv_pk_gal_1_8, 16);
 	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_1_10.bin", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize);
    write_cov(covfile, nelements, conv_pk_gal_1_10, 18);
 	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_1_12.bin", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize);
    write_cov(covfile, nelements, conv_pk_gal_1_12, 20);
	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_2_0.bin", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize);
    write_cov(covfile, nelements, conv_pk_gal_2_0, 8);
	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_2_2.bin", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize);
    write_cov(covfile, nelements, conv_pk_gal_2_2, 10);
	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_2_4.bin", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize);
    write_cov(covfile, nelements, conv_pk_gal_2_4, 12);
	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_2_6.bin", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize);
    write_cov(covfile, nelements, conv_pk_gal_2_6, 14);
 	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_2_8.bin", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize);
    write_cov(covfile, nelements, conv_pk_gal_2_8, 16);
 	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_2_10.bin", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize);
    write_cov(covfile, nelements, conv_pk_gal_2_10, 18);
 	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_2_12.bin", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize);
    write_cov(covfile, nelements, conv_pk_gal_2_12, 20);

	printf("Done dd covariance\n");

    for (i=0; i<nelements; i++) {
    	free(conv_pk_gal_0_0[i]);
    	free(conv_pk_gal_0_2[i]);
    	free(conv_pk_gal_0_4[i]);
    	free(conv_pk_gal_0_6[i]);
    	free(conv_pk_gal_0_8[i]);
    	free(conv_pk_gal_0_10[i]);
    	free(conv_pk_gal_0_12[i]);
    	free(conv_pk_gal_1_0[i]);
    	free(conv_pk_gal_1_2[i]);
    	free(conv_pk_gal_1_4[i]);
    	free(conv_pk_gal_1_6[i]);
    	free(conv_pk_gal_1_8[i]);
    	free(conv_pk_gal_1_10[i]);
    	free(conv_pk_gal_1_12[i]);
    	free(conv_pk_gal_2_0[i]);
    	free(conv_pk_gal_2_2[i]);
    	free(conv_pk_gal_2_4[i]);
    	free(conv_pk_gal_2_6[i]);
    	free(conv_pk_gal_2_8[i]);
    	free(conv_pk_gal_2_10[i]);
    	free(conv_pk_gal_2_12[i]);
    }
    free(conv_pk_gal_0_0);
    free(conv_pk_gal_0_2);
    free(conv_pk_gal_0_4);
    free(conv_pk_gal_0_6);
    free(conv_pk_gal_0_8);
    free(conv_pk_gal_0_10);
    free(conv_pk_gal_0_12);
    free(conv_pk_gal_1_0);
    free(conv_pk_gal_1_2);
    free(conv_pk_gal_1_4);
    free(conv_pk_gal_1_6);
    free(conv_pk_gal_1_8);
    free(conv_pk_gal_1_10);
    free(conv_pk_gal_1_12);
    free(conv_pk_gal_2_0);
    free(conv_pk_gal_2_2);
    free(conv_pk_gal_2_4);
    free(conv_pk_gal_2_6);
    free(conv_pk_gal_2_8);
    free(conv_pk_gal_2_10);
    free(conv_pk_gal_2_12);
	for (veltype=0; veltype<3; veltype++) {
		for (i=0; i<7; i++) {
			for (ell=0; ell<i+3; ell++) {
				gsl_spline_free (xi_dd_spline[veltype][i][ell]);
    			gsl_interp_accel_free (xi_dd_acc[veltype][i][ell]);
			}
		}
	}
	for (i=0; i<3; i++) {
