...
def linear_interpolation(conv_pk_array, sigma_u, num):
    ...
    for i in range(num):
        ...
        output.append((1.0-correction)*conv_pk_array[i][index_low] + correction*conv_pk_array[i][index_high])
...
#  fsigma8, sigma_v, bsigma8, b_add_sigma8, sigma_u = params
    fsigma8, sigma_v, bsigma8, b_add_sigma8, sigma_u, sigma_g = params
    ...
    for j in range(len(conv_pk_vel_gal_array)):
        conv_pk_vel_gal += conv_pk_vel_gal_array[i]*m*sigma_g**n
        n += 2
        if (n > 6):
            n = 0
            m = (fsigma8/fsigma8_old)**2
...
mock_num = int(sys.argv[5]) #Total number of mocks
...
gap_sigmau = 0.2
sigma_u_init = 0.2 # What initial sigma_u to use. 

expect_file = str('/data/s4479813/SDSS_random.csv')
...
datafile = str("/data/s4479813/2MTF_pkin.dat")

# chainfile = str('/data/s4479813/2MTF_vel_data_full/fit_pec_vel_2MTF_k0p%03d_0p%03d_gridcorr_%d_%d_%d_full.hdf5' % (int(1000.0*kmin), int(1000.0*kmax), gridsize, file_num, dot_num))
# chainfile = str('./fit_pec_vel_2MTF_k0p%03d_0p%03d_gridcorr__%d_%d_%d.hdf5' % (int(1000.0*kmin), int(1000.0*kmax), gridsize, file_num, dot_num))
chainfile = str('/data/s4479813/fit_pec_vel_2MTF_k0p%03d_0p%03d_gridcorr_%d_real.hdf5' % (int(1000.0*kmin), int(1000.0*kmax), gridsize))

conv_gg = str('/data/s4479813/conv_pk_vel_2MTF_k0p007_0p150_gal_gal_gridcorr20.dat')

# conv_vv = str('conv_pk_vel_2MTF_k0p007_0p150_gridcorr20_sigmau002.dat')
# conv_vv_ng = str('conv_pk_vel_2MTF_k0p007_0p150_gridcorr20_ng_sigmau002.dat')

conv_gg_add = str('/data/s4479813/conv_pk_2MTF_k0p150_0p950_gridcorr20_b_add.dat')
...
data_expect_all = np.array(pd.read_csv(expect_file, header=None, skiprow=1))
...
for i in range(len(data_expect)):
    ix = int(np.floor((data_expect[i,0]-xmin)/gridsize))
    iy = int(np.floor((data_expect[i,1]-ymin)/gridsize))
    iz = int(np.floor((data_expect[i,2]-zmin)/gridsize))
    if (ix == nx):
        ix = nx-1
    if (iy == ny):
        iy = ny-1
...
conv_vg = np.zeros(N_sigmau, 8)
conv_vv = np.zeros(N_sigmau, 1)
conv_gg = []
for i in range(N_sigmau):
    c = 1
    d = 0
    for j in range(8):
        str('/data/s4479813/fit_pec_vel_2MTF_k0p%03d_0p%03d_gridcorr_%d_real.hdf5' % (int(1000.0*kmin), int(1000.0*kmax), gridsize))
        data_file_conv_vg = str('/data/s4479813/wide_angle_covariance_k0p002_0p150_gridcorr20_dv_%d_%d_sigmau%03d.dat' %(c, d, int(10.0*i)))
        ...
        conv_vg[i][j] = conv_vg_final
        d += 2
        if (d > 7):
            c += 1
            d = 0
    
    data_file_conv_vv = str('/data/s4479813/wide_angle_covariance_k0p002_0p150_vv_gridcorr20_sigmau%03d.dat' %(int(10.0*i)))
    ...
    data_file_conv_vv_ng = str('/data/s4479813/wide_angle_covariance_k0p002_0p150_vv_ng_gridcorr20_sigmau%03d.dat' %(int(10.0*i)))
    ...
    for i in range(ncomp_velocity):
        conv_vv_final[i][i] += (conv_vv_ng_final[i][i] - conv_vv_final[i][i])/ngrid_SDSS_vel[i]
    conv_vv[i][0] = conv_vv_final

a = 0
b = 0
for k in range(21):
    data_file_conv_gg = str('/data/s4479813/wide_angle_covariance_k0p002_0p150_gridcorr20_dd_'+str(b)+'_'+str(a)+'.dat')
