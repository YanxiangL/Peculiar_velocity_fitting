...
def lnlike(params): 
    ...
    chi_squared = np.matmul(datagrid_SDSS.T, np.linalg.solve(conv_pk_new, datagrid_SDSS)) 
    #print(chi_squared)
...
comp_index = np.where(ngrid_SDSS > 0)[0]
distance_limit = sp.interpolate.splev(0.075, radial_spline, der=0)
distance = datagrid_vec[:,3]
comp_index_delete = np.intersect1d(np.where(distance>distance_limit)[0], comp_index)
...
comp_index = np.delete(comp_index, delete_index)
ncomp = len(comp_index)
datagrid_SDSS = datagrid_SDSS[comp_index]
errgrid_SDSS = errgrid_SDSS[comp_index]
datagrid_vec = datagrid_vec[comp_index,:]
remove_index_empty = np.where(ngrid_SDSS == 0)[0]
remove_index = np.union1d(remove_index_empty, comp_index_delete)
ngrid_SDSS = ngrid_SDSS[comp_index]
