...
def lnpost(params, fsigma8_old, sigma_y, ncomp, ngrid_comp, datagrid_comp, errgrid_comp, datagrid_vec, conv_pk):
    ...
    if not np.isfinite(prior):
        return -np.inf
    like = lnlike(params, fsigma8_old, sigma_y, ncomp, ngrid_comp, datagrid_comp, errgrid_comp, datagrid_vec, conv_pk)
    return prior + like
...
def lnlike(params, fsigma8_old, sigma_y, ncomp, ngrid_comp, datagrid_comp, errgrid_comp, datagrid_vec, conv_pk): 
    ...
    err_rand = datagrid_vec[0:,4]*sigma_varr/np.sqrt(ngrid_comp)
    conv_pk_diag = conv_pk_new[np.diag_indices(ncomp)]
    np.fill_diagonal(conv_pk_new, conv_pk_diag + errgrid_comp + err_rand**2)
...
sampler = emcee.EnsembleSampler(nwalkers, ndim, lambda *args: lnpost(*args), 
    args=[fsigma8_old, sigma_y, ncomp, ngrid_SDSS, datagrid_SDSS, errgrid_SDSS, datagrid_vec, conv_pk], backend=backend)
