...
double conv_integrand(double k, void * p) {
	...
	double gridcorr = 1.0;
	if (params.gridcorr == 1) gridcorr = gsl_spline_eval(gridcorr_spline, k, gridcorr_acc); //Interpolate the precomputed grid correction kernel. 
...
double conv_integral(double kmin, double kmax, double dist, int abtype, int ell, int pq, int gridcorr, int damping) {//This is the function calculates the integration. 
...
double H_dv(int ell, int p, double theta, double phi) {//The weighting function for the density-velocity cross-covariance matrix. 
...
double H_dd_0(int p, int q, double theta, double phi) {//The weighting function for the density auto-covariance matrix. 
...
int main(int argc, char **argv) {
	...
	double xmin = atof(argv[7]); //The xmin for the simulation. 
	double xmax = atof(argv[8]); //The xmax for the simulation.
	double ymin = atof(argv[9]); //The ymin for the simulation. 
    ...
    ngridcorr = 0;//load in the grid correction. 
	...
	double phi_diag = 0.0; 
	//At the diagonal of the matrix, line of sight to two galaxies overlap, so theta = 0, and phi = pi/2. This is because cos(phi) = s dot d = 0, it is set to zero here because it is irrelavent when calculating the diagonal;
	
	double diag_vv = conv_integral(kmin, kmax, 0.0, 2, 0, -2, 1, 2)*H_vv(0, theta_diag, phi_diag);
	double diag_vv_ng = conv_integral(kmin, kmax, 0.0, 2, 0, -2, 0, 2)*H_vv(0, theta_diag, phi_diag);
	...
	for (veltype=0; veltype<2; veltype++) { //veltype = 0 for non-gridded velocity auto-covariance matrix, veltype = 1 for the gridded version.
		for (ell=0; ell<2; ell++) { //ell is the order of the spherical bessel function.
			for (j=0; j<rbins; j++) xivals[j] = conv_integral(kmin, kmax, svals[j], 2, 2*ell, -2, veltype, 2);
			printf("%d, %d\n", veltype, 2*ell);
    ...
    for (i=0; i<niter; i++) {
        ...
        if (indi == indj) {
            ...
            conv_pk_vel[indi][indj] = 10000.0*cov/(2.0*M_PI*M_PI)*datagrid_dm[indi]*datagrid_dm[indj];//The 10000.0 is H^2. Here we assume all unit are in Mpc/h, so H = 100. 
	...
	for (veltype=0; veltype<2; veltype++) {
		for (i=0; i<4; i++) {//2*i-1 is the power of k in the integral.
			for (ell=0; ell<i+2; ell++) {
				for (j=0; j<rbins; j++) xivals[j] = conv_integral(kmin, kmax, svals[j], veltype+1, 2*ell+1, 2*i-1, 1, 1); //veltype+1 gives determine which power spectrum to use.
    ...
    for (i=0; i<niter; i++) {
        ...
        if (indi == indj) {
			...
			double cov_1_0_opp = 0, cov_1_2_opp = 0, cov_1_4_opp = 0, cov_1_6_opp = 0;//opp here means the transpose of the corresponding covariance matrix. 
