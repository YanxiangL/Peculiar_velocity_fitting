...
double conv_integrand(double k, void * p) {
	...
	double gridcorr = 1.0;
	if (params.gridcorr) gridcorr = gsl_spline_eval(gridcorr_spline, k, gridcorr_acc);
...
int main(int argc, char **argv) {
	...
	//int job_num = atoi(argv[6]);	// The job number of getafix
	...
	double diag_mm_0_12 = conv_integral(kmin, kmax, 0.0, 0, 0, 12, 1)*H_dd(0, 3, 3, theta_diag, phi_diag);
	double diag_mm_0_10 = conv_integral(kmin, kmax, 0.0, 0, 0, 10, 1)*(H_dd(0, 3, 2, theta_diag, phi_diag) + H_dd(0, 2, 3, theta_diag, phi_diag));
	double diag_mm_0_8  = conv_integral(kmin, kmax, 0.0, 0, 0, 8, 1)*(H_dd(0, 3, 1, theta_diag, phi_diag)/6.0 + H_dd(0, 2, 2, theta_diag, phi_diag)/4.0 + H_dd(0, 1, 3, theta_diag, phi_diag)/6.0);
	double diag_mm_0_6  = conv_integral(kmin, kmax, 0.0, 0, 0, 6, 1)*(H_dd(0, 3, 0, theta_diag, phi_diag)/6.0 + H_dd(0, 2, 1, theta_diag, phi_diag)/2.0 + H_dd(0, 1, 2, theta_diag, phi_diag)/2.0 + H_dd(0, 0, 3, theta_diag, phi_diag)/6.0);
	double diag_mm_0_4  = conv_integral(kmin, kmax, 0.0, 0, 0, 4, 1)*(H_dd(0, 2, 0, theta_diag, phi_diag)/2.0 + H_dd(0, 1, 1, theta_diag, phi_diag) + H_dd(0, 0, 2, theta_diag, phi_diag)/2.0);
	double diag_mm_0_2  = conv_integral(kmin, kmax, 0.0, 0, 0, 2, 1)*(H_dd(0, 1, 0, theta_diag, phi_diag) + H_dd(0, 0, 1, theta_diag, phi_diag));
	double diag_mm_0_0  = conv_integral(kmin, kmax, 0.0, 0, 0, 0, 1)*H_dd(0, 0, 0, theta_diag, phi_diag);
	double diag_mm_1_12 = conv_integral(kmin, kmax, 0.0, 0, 0, 12, 1)*(H_dd(0, 4, 3, theta_diag, phi_diag) + H_dd(0, 3, 4, theta_diag, phi_diag));
	double diag_mm_1_10 = conv_integral(kmin, kmax, 0.0, 0, 0, 10, 1)*(H_dd(0, 4, 2, theta_diag, phi_diag) + 2.0*H_dd(0, 3, 3, theta_diag, phi_diag) + H_dd(0, 2, 4, theta_diag, phi_diag));
	double diag_mm_1_8  = conv_integral(kmin, kmax, 0.0, 0, 0, 8, 1)*(H_dd(0, 4, 1, theta_diag, phi_diag)/6.0 + 5.0*H_dd(0, 3, 2, theta_diag, phi_diag)/12.0 + 5.0*H_dd(0, 2, 3, theta_diag, phi_diag)/12.0 + H_dd(0, 1, 4, theta_diag, phi_diag)/6.0);
	double diag_mm_1_6  = conv_integral(kmin, kmax, 0.0, 0, 0, 6, 1)*(H_dd(0, 4, 0, theta_diag, phi_diag)/6.0 + 2.0*H_dd(0, 3, 1, theta_diag, phi_diag)/3.0 + H_dd(0, 2, 2, theta_diag, phi_diag) + 2.0*H_dd(0, 1, 3, theta_diag, phi_diag)/3.0 + H_dd(0, 0, 4, theta_diag, phi_diag)/6.0);
	double diag_mm_1_4  = conv_integral(kmin, kmax, 0.0, 0, 0, 4, 1)*(H_dd(0, 3, 0, theta_diag, phi_diag)/2.0 + 3.0*H_dd(0, 2, 1, theta_diag, phi_diag)/2.0 + 3.0*H_dd(0, 1, 2, theta_diag, phi_diag)/2.0 + H_dd(0, 0, 3, theta_diag, phi_diag)/2.0);
	double diag_mm_1_2  = conv_integral(kmin, kmax, 0.0, 0, 0, 2, 1)*(H_dd(0, 2, 0, theta_diag, phi_diag) + 2.0*H_dd(0, 1, 1, theta_diag, phi_diag) + H_dd(0, 0, 2, theta_diag, phi_diag));
	double diag_mm_1_0  = conv_integral(kmin, kmax, 0.0, 0, 0, 0, 1)*(H_dd(0, 1, 0, theta_diag, phi_diag) + H_dd(0, 0, 1, theta_diag, phi_diag));
	double diag_mm_2_12 = conv_integral(kmin, kmax, 0.0, 0, 0, 12, 1)*H_dd(0, 4, 4, theta_diag, phi_diag);
	double diag_mm_2_10 = conv_integral(kmin, kmax, 0.0, 0, 0, 10, 1)*(H_dd(0, 4, 3, theta_diag, phi_diag) + H_dd(0, 3, 4, theta_diag, phi_diag));
	double diag_mm_2_8  = conv_integral(kmin, kmax, 0.0, 0, 0, 8, 1)*(H_dd(0, 4, 2, theta_diag, phi_diag)/6.0 + H_dd(0, 3, 3, theta_diag, phi_diag)/4.0 + H_dd(0, 2, 4, theta_diag, phi_diag)/6.0);
	double diag_mm_2_6  = conv_integral(kmin, kmax, 0.0, 0, 0, 6, 1)*(H_dd(0, 4, 1, theta_diag, phi_diag)/6.0 + H_dd(0, 3, 2, theta_diag, phi_diag)/2.0 + H_dd(0, 2, 3, theta_diag, phi_diag)/2.0 + H_dd(0, 1, 4, theta_diag, phi_diag)/6.0);
	double diag_mm_2_4  = conv_integral(kmin, kmax, 0.0, 0, 0, 4, 1)*(H_dd(0, 3, 1, theta_diag, phi_diag)/2.0 + H_dd(0, 2, 2, theta_diag, phi_diag) + H_dd(0, 1, 3, theta_diag, phi_diag)/2.0);
	double diag_mm_2_2  = conv_integral(kmin, kmax, 0.0, 0, 0, 2, 1)*(H_dd(0, 2, 1, theta_diag, phi_diag) + H_dd(0, 1, 2, theta_diag, phi_diag));
	double diag_mm_2_0  = conv_integral(kmin, kmax, 0.0, 0, 0, 0, 1)*H_dd(0, 1, 1, theta_diag, phi_diag);
    ...
    for (i=0; i<niter; i++) {
        ...
        if (indi == indj) {
			...
			for (ell=0; ell<4; ell+=2) {
				cov += creal(cpow(I,ell+2))*gsl_spline_eval(xi_vv_spline[1][ell/2], s, xi_vv_acc[1][ell/2])*H_vv(ell, theta, phi);
				cov_ng += creal(cpow(I,ell+2))*gsl_spline_eval(xi_vv_spline[0][ell/2], s, xi_vv_acc[0][ell/2])*H_vv(ell, theta, phi);
			}
	...
	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_sigmau%03d.dat", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize, (int) (10.0*sigma_u));
	...
	for (i=0; i<rbins; i++) svals[i] = i*(smax-smin)/(rbins-1.0) + smin;
	for (veltype=0; veltype<2; veltype++) {
		for (i=0; i<4; i++) {//2*j-1 is the power of k in the integral. 
	...
	for (veltype=0; veltype<3; veltype++) { //veltype determines which power spectrum to use. 
		for (i=0; i<7; i++) {// 2*i determines the power of k in the integral. 
			for (ell=0; ell<i+3; ell++) { //2*ell is the order of the spherical bessel function. 
				for (j=0; j<rbins; j++) xivals[j] = conv_integral(kmin, kmax, svals[j], veltype, 2*ell, 2*i, 1);
				printf("%d, %d, %d\n", veltype, 2*i, 2*ell);
	...
	for (i=0; i<niter; i++) {
        ...
        int indj = indexj[i];
        if (indi == indj) {
            conv_pk_gal_0_0[indi][indi]  =  diag_mm_0_0;
            conv_pk_gal_0_2[indi][indi]  = -diag_mm_0_2/2.0;
            conv_pk_gal_0_4[indi][indi]  =  diag_mm_0_4/4.0;
            conv_pk_gal_0_6[indi][indi]  = -diag_mm_0_6/8.0;
            conv_pk_gal_0_8[indi][indi]  =  diag_mm_0_8/16.0;
            conv_pk_gal_0_10[indi][indi] = -diag_mm_0_10/384.0;
            conv_pk_gal_0_12[indi][indi] =  diag_mm_0_12/2304.0;
            conv_pk_gal_1_0[indi][indi]  =  diag_mm_1_0;
            conv_pk_gal_1_2[indi][indi]  = -diag_mm_1_2/2.0;
            conv_pk_gal_1_4[indi][indi]  =  diag_mm_1_4/4.0;
            conv_pk_gal_1_6[indi][indi]  = -diag_mm_1_6/8.0;
            conv_pk_gal_1_8[indi][indi]  =  diag_mm_1_8/16.0;
            conv_pk_gal_1_10[indi][indi] = -diag_mm_1_10/384.0;
            conv_pk_gal_1_12[indi][indi] =  diag_mm_1_12/2304.0;
            conv_pk_gal_2_0[indi][indi]  =  diag_mm_2_0;
            conv_pk_gal_2_2[indi][indi]  = -diag_mm_2_2/2.0;
            conv_pk_gal_2_4[indi][indi]  =  diag_mm_2_4/4.0;
            conv_pk_gal_2_6[indi][indi]  = -diag_mm_2_6/8.0;
            conv_pk_gal_2_8[indi][indi]  =  diag_mm_2_8/16.0;
            conv_pk_gal_2_10[indi][indi] = -diag_mm_2_10/384.0;
            conv_pk_gal_2_12[indi][indi] =  diag_mm_2_12/2304.0;
            printf("%d, %lf, %lf, %lf, %lf, %lf, %lf\n", indi, datagrid_x[indi], datagrid_y[indi], datagrid_z[indi], 1.0e6*conv_pk_gal_0_4[indi][indi], 1.0e6*conv_pk_gal_1_6[indi][indi], 1.0e6*conv_pk_gal_2_12[indi][indi]);
        } else {
            double xi = datagrid_x[indi];
            double yi = datagrid_y[indi];
            double zi = datagrid_z[indi];
	...
	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_0_0_sigmau%03d.dat", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize, (int) (10.0*sigma_u));
    write_cov(covfile, nelements, conv_pk_gal_0_0);
	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_0_2_sigmau%03d.dat", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize, (int) (10.0*sigma_u));
    write_cov(covfile, nelements, conv_pk_gal_0_2);
	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_0_4_sigmau%03d.dat", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize, (int) (10.0*sigma_u));
    write_cov(covfile, nelements, conv_pk_gal_0_4);
	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_0_6_sigmau%03d.dat", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize, (int) (10.0*sigma_u));
    write_cov(covfile, nelements, conv_pk_gal_0_6);
 	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_0_8_sigmau%03d.dat", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize, (int) (10.0*sigma_u));
    write_cov(covfile, nelements, conv_pk_gal_0_8);
 	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_0_10_sigmau%03d.dat", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize, (int) (10.0*sigma_u));
    write_cov(covfile, nelements, conv_pk_gal_0_10);
 	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_0_12_sigmau%03d.dat", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize, (int) (10.0*sigma_u));
    write_cov(covfile, nelements, conv_pk_gal_0_12);
	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_1_0_sigmau%03d.dat", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize, (int) (10.0*sigma_u));
    write_cov(covfile, nelements, conv_pk_gal_1_0);
	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_1_2_sigmau%03d.dat", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize, (int) (10.0*sigma_u));
    write_cov(covfile, nelements, conv_pk_gal_1_2);
	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_1_4_sigmau%03d.dat", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize, (int) (10.0*sigma_u));
    write_cov(covfile, nelements, conv_pk_gal_1_4);
	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_1_6_sigmau%03d.dat", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize, (int) (10.0*sigma_u));
    write_cov(covfile, nelements, conv_pk_gal_1_6);
 	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_1_8_sigmau%03d.dat", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize, (int) (10.0*sigma_u));
    write_cov(covfile, nelements, conv_pk_gal_1_8);
 	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_1_10_sigmau%03d.dat", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize, (int) (10.0*sigma_u));
    write_cov(covfile, nelements, conv_pk_gal_1_10);
 	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_1_12_sigmau%03d.dat", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize, (int) (10.0*sigma_u));
    write_cov(covfile, nelements, conv_pk_gal_1_12);
	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_2_0_sigmau%03d.dat", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize, (int) (10.0*sigma_u));
    write_cov(covfile, nelements, conv_pk_gal_2_0);
	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_2_2_sigmau%03d.dat", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize, (int) (10.0*sigma_u));
    write_cov(covfile, nelements, conv_pk_gal_2_2);
	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_2_4_sigmau%03d.dat", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize, (int) (10.0*sigma_u));
    write_cov(covfile, nelements, conv_pk_gal_2_4);
	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_2_6_sigmau%03d.dat", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize, (int) (10.0*sigma_u));
    write_cov(covfile, nelements, conv_pk_gal_2_6);
 	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_2_8_sigmau%03d.dat", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize, (int) (10.0*sigma_u));
    write_cov(covfile, nelements, conv_pk_gal_2_8);
 	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_2_10_sigmau%03d.dat", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize, (int) (10.0*sigma_u));
    write_cov(covfile, nelements, conv_pk_gal_2_10);
 	sprintf(covfile, "%s_k0p%03d_0p%03d_gridcorr%02d_dd_2_12_sigmau%03d.dat", covfile_base, (int)(1000.0*kmin), (int)(1000.0*kmax), gridsize, (int) (10.0*sigma_u));
    write_cov(covfile, nelements, conv_pk_gal_2_12);
