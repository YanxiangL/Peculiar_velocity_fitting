...
int main(int argc, char **argv) {
	...
	double diag_vv = conv_integral(kmin, kmax, 0.0, 2, 0, -2, 1)*H_vv(0, 1.0, 0.0);
	double diag_vv_ng = conv_integral(kmin, kmax, 0.0, 2, 0, -2, 0)*H_vv(0, 1.0, 0.0);
	double diag_mm_0_12 = conv_integral(kmin, kmax, 0.0, 0, 0, 12, 1)*H_dd(0, 3, 3, 1.0, 0.0);
	double diag_mm_0_10 = conv_integral(kmin, kmax, 0.0, 0, 0, 10, 1)*(H_dd(0, 3, 2, 1.0, 0.0) + H_dd(0, 2, 3, 1.0, 0.0));
	double diag_mm_0_8  = conv_integral(kmin, kmax, 0.0, 0, 0, 8, 1)*(H_dd(0, 3, 1, 1.0, 0.0)/6.0 + H_dd(0, 2, 2, 1.0, 0.0)/4.0 + H_dd(0, 1, 3, 1.0, 0.0)/6.0);
	double diag_mm_0_6  = conv_integral(kmin, kmax, 0.0, 0, 0, 6, 1)*(H_dd(0, 3, 0, 1.0, 0.0)/6.0 + H_dd(0, 2, 1, 1.0, 0.0)/2.0 + H_dd(0, 1, 2, 1.0, 0.0)/2.0 + H_dd(0, 0, 3, 1.0, 0.0)/6.0);
	double diag_mm_0_4  = conv_integral(kmin, kmax, 0.0, 0, 0, 4, 1)*(H_dd(0, 2, 0, 1.0, 0.0)/2.0 + H_dd(0, 1, 1, 1.0, 0.0) + H_dd(0, 0, 2, 1.0, 0.0)/2.0);
	double diag_mm_0_2  = conv_integral(kmin, kmax, 0.0, 0, 0, 2, 1)*(H_dd(0, 1, 0, 1.0, 0.0) + H_dd(0, 0, 1, 1.0, 0.0));
	double diag_mm_0_0  = conv_integral(kmin, kmax, 0.0, 0, 0, 0, 1)*H_dd(0, 0, 0, 1.0, 0.0);
	double diag_mm_1_12 = conv_integral(kmin, kmax, 0.0, 0, 0, 12, 1)*(H_dd(0, 4, 3, 1.0, 0.0) + H_dd(0, 3, 4, 1.0, 0.0));
	double diag_mm_1_10 = conv_integral(kmin, kmax, 0.0, 0, 0, 10, 1)*(H_dd(0, 4, 2, 1.0, 0.0) + 2.0*H_dd(0, 3, 3, 1.0, 0.0) + H_dd(0, 2, 4, 1.0, 0.0));
	double diag_mm_1_8  = conv_integral(kmin, kmax, 0.0, 0, 0, 8, 1)*(H_dd(0, 4, 1, 1.0, 0.0)/6.0 + 5.0*H_dd(0, 3, 2, 1.0, 0.0)/12.0 + 5.0*H_dd(0, 2, 3, 1.0, 0.0)/12.0 + H_dd(0, 1, 4, 1.0, 0.0)/6.0);
	double diag_mm_1_6  = conv_integral(kmin, kmax, 0.0, 0, 0, 6, 1)*(H_dd(0, 4, 0, 1.0, 0.0)/6.0 + 2.0*H_dd(0, 3, 1, 1.0, 0.0)/3.0 + H_dd(0, 2, 2, 1.0, 0.0) + 2.0*H_dd(0, 1, 3, 1.0, 0.0)/3.0 + H_dd(0, 0, 4, 1.0, 0.0)/6.0);
	double diag_mm_1_4  = conv_integral(kmin, kmax, 0.0, 0, 0, 4, 1)*(H_dd(0, 3, 0, 1.0, 0.0)/2.0 + 3.0*H_dd(0, 2, 1, 1.0, 0.0)/2.0 + 3.0*H_dd(0, 1, 2, 1.0, 0.0)/2.0 + H_dd(0, 0, 3, 1.0, 0.0)/2.0);
	double diag_mm_1_2  = conv_integral(kmin, kmax, 0.0, 0, 0, 2, 1)*(H_dd(0, 2, 0, 1.0, 0.0) + 2.0*H_dd(0, 1, 1, 1.0, 0.0) + H_dd(0, 0, 2, 1.0, 0.0));
	double diag_mm_1_0  = conv_integral(kmin, kmax, 0.0, 0, 0, 0, 1)*(H_dd(0, 1, 0, 1.0, 0.0) + H_dd(0, 0, 1, 1.0, 0.0));
	double diag_mm_2_12 = conv_integral(kmin, kmax, 0.0, 0, 0, 12, 1)*H_dd(0, 4, 4, 1.0, 0.0);
	double diag_mm_2_10 = conv_integral(kmin, kmax, 0.0, 0, 0, 10, 1)*(H_dd(0, 4, 3, 1.0, 0.0) + H_dd(0, 3, 4, 1.0, 0.0));
	double diag_mm_2_8  = conv_integral(kmin, kmax, 0.0, 0, 0, 8, 1)*(H_dd(0, 4, 2, 1.0, 0.0)/6.0 + H_dd(0, 3, 3, 1.0, 0.0)/4.0 + H_dd(0, 2, 4, 1.0, 0.0)/6.0);
	double diag_mm_2_6  = conv_integral(kmin, kmax, 0.0, 0, 0, 6, 1)*(H_dd(0, 4, 1, 1.0, 0.0)/6.0 + H_dd(0, 3, 2, 1.0, 0.0)/2.0 + H_dd(0, 2, 3, 1.0, 0.0)/2.0 + H_dd(0, 1, 4, 1.0, 0.0)/6.0);
	double diag_mm_2_4  = conv_integral(kmin, kmax, 0.0, 0, 0, 4, 1)*(H_dd(0, 3, 1, 1.0, 0.0)/2.0 + H_dd(0, 2, 2, 1.0, 0.0) + H_dd(0, 1, 3, 1.0, 0.0)/2.0);
	double diag_mm_2_2  = conv_integral(kmin, kmax, 0.0, 0, 0, 2, 1)*(H_dd(0, 2, 1, 1.0, 0.0) + H_dd(0, 1, 2, 1.0, 0.0));
	double diag_mm_2_0  = conv_integral(kmin, kmax, 0.0, 0, 0, 0, 1)*H_dd(0, 1, 1, 1.0, 0.0);
    ...
    for (i=0; i<niter; i++) {
        ...
        if (indi == indj) {
			...
			for (ell=0; ell<4; ell+=2) {
				cov += creal(cpow(I,ell+2))*gsl_spline_eval(xi_vv_spline[1][ell/2], s, xi_vv_acc[1][ell/2]);
				cov_ng += creal(cpow(I,ell+2))*gsl_spline_eval(xi_vv_spline[0][ell/2], s, xi_vv_acc[0][ell/2]);
			}
