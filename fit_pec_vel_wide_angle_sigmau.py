...
def get_dL(params_fid):
    ...
    ffid, sigma_v_fid, bfid, baddfid, sigmag_fid, sigma_u_fid = params_fid
    
    sigma_u_up = np.ceil(sigma_u_fid, dtype=np.int64)
    sigma_u_down = np.floor(sigma_u_fid, dtype = np.int64)
    # sigmag_fid = sigma_g
    conv_vg_dash = []
    conv_vv_dash = []
    conv_vg_derivative_dash = []
    conv_vv_derivative_dash = []
    for i in range(2):
        if (i == 0):
            sigma_u = sigma_u_down
        else:
            sigma_u = sigma_u_up
        c = 1
        d = 0
        conv_vg_sigma_u = []
        conv_vg_sigma_u_derivative = []
        for j in range(8):
            ...
            data_file_conv_vg_derivative = str('/data/s4479813/wide_angle_covariance_k0p002_0p%03d_gridcorr20_dv_%d_%d_sigmau%03d_derivative.dat' %(int(1000.0*kmax_velocity), c, d, int(10.0*sigma_u)))
            print(data_file_conv_vg_derivative)
            conv_vg_derivative_element = np.array(pd.read_csv(data_file_conv_vg_derivative, delim_whitespace=True, header=None, skiprows=1))/10**(8+d)
            conv_vg_derivative_final = np.delete(np.delete(conv_vg_derivative_element, remove_velocity, axis = 0), remove_galaxy, axis = 1)
            conv_vg_sigma_u_derivative.append(conv_vg_derivative_final)
            d += 2
            if (d > 7):
                c += 1
                d = 0
                
        conv_vg_dash.append(conv_vg_sigma_u)
        conv_vg_derivative_dash.append(conv_vg_sigma_u_derivative)
        ...
        data_file_conv_vv_derivative = str('/data/s4479813/wide_angle_covariance_k0p002_0p%03d_gridcorr20_vv_sigmau%03d_derivative.dat' %(int(1000.0*kmax_velocity), int(10.0*sigma_u)))
        conv_vv_derivative_element = np.array(pd.read_csv(data_file_conv_vv_derivative, delim_whitespace=True, header=None, skiprows=1))/1.0e6
        conv_vv_derivative_final = np.delete(np.delete(conv_vv_derivative_element, remove_velocity, axis = 0), remove_velocity, axis = 1)
           
        data_file_conv_vv_derivative_ng = str('/data/s4479813/wide_angle_covariance_k0p002_0p%03d_gridcorr20_vv_ng_sigmau%03d_derivative.dat' %(int(1000.0*kmax_velocity), int(10.0*sigma_u)))
        conv_vv_derivative_ng_element = np.array(pd.read_csv(data_file_conv_vv_derivative_ng, delim_whitespace=True, header=None, skiprows=1))/1.0e6
        conv_vv_derivative_ng_final = np.delete(np.delete(conv_vv_derivative_ng_element, remove_velocity, axis = 0), remove_velocity, axis = 1)
        
        for k in range(ncomp_velocity):
            conv_vv_final[k][k] += (conv_vv_ng_final[k][k] - conv_vv_final[k][k])/ngrid_SDSS_vel[k]
            conv_vv_derivative_final[k][k] += (conv_vv_derivative_ng_final[k][k] - conv_vv_derivative_final[k][k])/ngrid_SDSS_vel[k]
        conv_vv_dash.append(conv_vv_final)
        conv_vv_derivative_dash.append(conv_vv_derivative_final)
    
    correction = sigma_u_fid - sigma_u_down
    
    conv_vv = []
    conv_vv_derivative = []
    conv_vv.append(correction*conv_vv_dash[1] + (1.0-correction)*conv_vv_dash[0])
    conv_vv_derivative.append(correction*conv_vv_derivative_dash[1] + (1.0 - correction)*conv_vv_derivative_dash[0])
    
    conv_vg = []
    conv_vg_derivative = []
    for i in range(8):
        conv_vg.append(correction*conv_vg_dash[1][i] + (1.0-correction)*conv_vg_dash[0][i])
        conv_vg_derivative.append(correction*conv_vg_derivative_dash[1][i] + (1.0-correction)*conv_vg_derivative_dash[0][i])
    ...
    conv_bf_derivative = np.empty((len(datagrid_comp), len(datagrid_comp), 3))
    conv_f_derivative = np.empty((len(datagrid_comp), len(datagrid_comp), 3))
    for i in range(7):
    ...
    conv_bf_derivative[:,:,0] = np.concatenate((np.concatenate((zeros_gg, conv_vg_derivative[0].T), axis=1), np.concatenate((conv_vg_derivative[0], zeros_vv), axis=1)), axis = 0)
    conv_bf_derivative[:,:,1] = np.concatenate((np.concatenate((zeros_gg, conv_vg_derivative[1].T), axis=1), np.concatenate((conv_vg_derivative[1], zeros_vv), axis=1)), axis = 0)
    conv_bf_derivative[:,:,2] = np.concatenate((np.concatenate((zeros_gg, conv_vg_derivative[2].T), axis=1), np.concatenate((conv_vg_derivative[2], zeros_vv), axis=1)), axis = 0)
    conv_bf_derivative[:,:,3] = np.concatenate((np.concatenate((zeros_gg, conv_vg_derivative[3].T), axis=1), np.concatenate((conv_vg_derivative[3], zeros_vv), axis=1)), axis = 0)

    conv_f_derivative[:,:,0] = np.concatenate((np.concatenate((zeros_gg, conv_vg_derivative[4].T), axis=1), np.concatenate((conv_vg_derivative[4], conv_vv[0]), axis=1)), axis = 0)
    conv_f_derivative[:,:,1] = np.concatenate((np.concatenate((zeros_gg, conv_vg_derivative[5].T), axis=1), np.concatenate((conv_vg_derivative[5], zeros_vv), axis=1)), axis = 0)
    conv_f_derivative[:,:,2] = np.concatenate((np.concatenate((zeros_gg, conv_vg_derivative[6].T), axis=1), np.concatenate((conv_vg_derivative[6], zeros_vv), axis=1)), axis = 0)
    conv_f_derivative[:,:,3] = np.concatenate((np.concatenate((zeros_gg, conv_vg_derivative[7].T), axis=1), np.concatenate((conv_vg_derivative[7], zeros_vv), axis=1)), axis = 0)
    
    # Compute derivatives about fiducial values
    sigmag_array = np.array([sigmag_fid**(2*n) for n in range(0,7)])
    sigmag_array_derivative = np.array([sigmag_fid**(2*n) for n in range(0,3)])
    ...
    dCdsigmau = np.sum(sigmag_array_derivative*(ffid**2*conv_f_derivative + bfid*ffid*conv_bf_derivative), axis=-1)
    ...
    dLdsigmau = np.sum(np.diag(np.matmul(dLdC, dCdsigmau)))
    ...
    Fisher_dCdsigmau = np.matmul(np.matmul(Cfid_inv, dCdsigmau), Cfid_inv)
    ...
    d2Ldbdsigmau = 0.5*np.sum(np.einsum('ij,ji->i', Fisher_dCdb, dCdsigmau))
    ...
    d2Ldfdsigmau = 0.5*np.sum(np.einsum('ij,ji->i', Fisher_dCdf, dCdsigmau))
    ...
    d2Ldbadddsigmau = 0.5*np.sum(np.einsum('ij,ji->i', Fisher_dCdbadd, dCdsigmau))
    ...
    d2Ldsigmavdsigmau = 0.5*np.sum(np.einsum('ij,ji->i', Fisher_dCdsigmav, dCdsigmau))
    d2Ldsigmag2 = 0.5*np.sum(np.einsum('ij,ji->i', Fisher_dCdsigmag, dCdsigmag))
    d2Ldsigmagdsigmau = 0.5*np.sum(np.einsum('ij,ji->i', Fisher_dCdsigmag, dCdsigmau))
    d2Ldsigmau2 = 0.5*np.sum(np.einsum('ij,ji->i', Fisher_dCdsigmau, dCdsigmau))
    ...
    dL = np.array([dLdf, dLdsigmav, dLdb, dLdbadd, dLdsigmag, dLdsigmau])
    ...
    d2L = np.array([[d2Ldf2, d2Ldfdsigmav, d2Ldbdf, d2Ldfdbadd, d2Ldfdsigmag, d2Ldfdsigmau], 
            [d2Ldfdsigmav, d2Ldsigmav2, d2Ldbdsigmav, d2Ldbadddsigmav, d2Ldsigmavdsigmag, d2Ldsigmavdsigmau], 
            [d2Ldbdf, d2Ldbdsigmav, d2Ldb2, d2Ldbdbadd, d2Ldbdsigmag, d2Ldbdsigmau], 
            [d2Ldfdbadd, d2Ldbadddsigmav, d2Ldbdbadd,  d2Ldbadd2, d2Ldbadddsigmag, d2Ldbadddsigmau],
            [d2Ldfdsigmag, d2Ldsigmavdsigmag, d2Ldbdsigmag, d2Ldbadddsigmag, d2Ldsigmag2, d2Ldsigmagdsigmau], 
            [d2Ldfdsigmau, d2Ldsigmavdsigmau, d2Ldbdsigmau, d2Ldbadddsigmau, d2Ldsigmagdsigmau, d2Ldsigmau2]])
...
#    fsigma8, sigma_v, bsigma8, b_add_sigma8, sigma_u = params
    ...
    fsigma8, sigma_v, bsigma8, b_add_sigma8, sigma_g, sigma_u = params
...
def lnlike(params): 
    ...
    diffs = np.array([fsigma8/fsigma8_old, sigma_v, bsigma8/bsigma8_old, b_add_sigma8/bsigma8_old, sigma_g, sigma_u]) - params_fid
...
correction = int(sys.argv[6]) #Enter 0 for non-corrected log-distance ratio, 1 for corrected log-distance ratio. 
process_num = int(sys.argv[7]) #This is the number of processors you want to use. 
sigma_g = int(sys.argv[8])  #This is the value of sigma_g to use. 
kmax_velocity = float(sys.argv[9])
...
if (correction == 0):
    chainfile = str('/data/s4479813/SDSS_data/fit_pec_vel_SDSS_k0p%03d_0p%03d_gridcorr_%d_%d_full_no_correction_Taylor_all.hdf5' % (int(1000.0*kmin), int(1000.0*kmax_velocity), gridsize, mock_num))
elif (correction == 1):
    chainfile = str('/data/s4479813/SDSS_data/fit_pec_vel_SDSS_k0p%03d_0p%03d_gridcorr_%d_%d_full_correction_Taylor_all.hdf5' % (int(1000.0*kmin), int(1000.0*kmax_velocity), gridsize, mock_num))
...
if __name__ == "__main__":
    ...
    params_fid = np.array([ffid, sigma_v_fid, bfid, baddfid, sigmag_fid, sigma_u_fid])
    converged = 1
    while(~converged):
        ...
        fvals, sigmavvals, bvals, baddvals, sigmagvals, sigmauvals = (0.0, 1.0), (1.0, 1000.0), (0.0, 3.0), (0.0, 10.0), (0.0, 10.0), (0.0, 25.0)
        result = differential_evolution(lambda *args: -lnpost(*args), bounds=(fvals, sigmavvals, bvals, baddvals, sigmagvals, sigmauvals), maxiter=10000, tol=1.0e-6)
        print("#-------------- Best-fit----------------")
        print(result["x"])

        params_fid_new = np.array([result["x"][0]/fsigma8_old, result["x"][1], result["x"][2]/bsigma8_old, result["x"][3]/bsigma8_old, result["x"][4]], result["x"][4])
    ...
    begin = [[1.0*np.random.rand(), 999.0*np.random.rand()+1.0, 3.0*np.random.rand(), 10.0*np.random.rand(), 10.0*np.random.rand(), 25.0*np.random.rand()] for i in range(nwalkers)]
